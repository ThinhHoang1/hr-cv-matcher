    ...
    static async processFile(userId: string, filePath: string): Promise<ProcessedResult> {
        ...
        try {
            ...
            const inferDepartment = (summary: string, skills: string[]): string => {
                ...
                if (text.includes('marketing') || text.includes('seo') || text.includes('content') || text.includes('social media') || text.includes('brand')) return 'Marketing';
                if (text.includes('sales') || text.includes('business development') || text.includes('account manager') || text.includes('b2b')) return 'Sales';
                if (text.includes('hr') || text.includes('recruitment') || text.includes('human resources') || text.includes('talent')) return 'HR';
                if (text.includes('finance') || text.includes('accounting') || text.includes('audit') || text.includes('tax')) return 'Finance';
                if (text.includes('operations') || text.includes('logistics') || text.includes('supply chain') || text.includes('admin')) return 'Operations';
                return 'Technology';
            };
...
`.trim();
                console.log(`📝 Created fallback raw_text: ${rawText.length} characters`);
            }
...
`.trim();
            ...
            console.log(`💾 Saving candidate with raw_text: ${candidateRecord.raw_text?.length || 0} chars`);

            // 7. Upsert Candidate with raw text for RAG retrieval
            ...
            console.log(`✅ Saved candidate: ${candidate?.name}, raw_text in DB: ${candidate?.raw_text?.length || 0} chars`);
